::mod_bombs.HooksMod.hook("scripts/items/tools/daze_bomb_item", function(q) {
	...
	q.onEquip = @(__original) function(){
		__original();
		local skill = this.getContainer().getActor().getSkills().getSkillByID("actives.throw_daze_bomb");
		skill.m.FreeCounter++;		
	}
	...
	{	
  		this.addSkill(::new("scripts/skills/actives/rf_sling_daze_bomb_skill_mb"));  		
  		this.getContainer().getActor().getSkills().getSkillByID("actives.sling_daze_bomb").m.FreeCounter++;
	}}.onPutIntoBag;

	q.onRemovedFromBag = @(__original) function()
	{
		local skill = this.getContainer().getActor().getSkills().getSkillByID("actives.sling_daze_bomb");						
