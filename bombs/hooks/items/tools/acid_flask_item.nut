::mod_bombs.HooksMod.hook("scripts/items/tools/acid_flask_item", function(q) {
	...
	q.onEquip = @(__original) function(){
		__original();
		local skill = this.getContainer().getActor().getSkills().getSkillByID("actives.throw_acid_flask");
		skill.m.FreeCounter++;		
	}
	...
	{	
  		this.addSkill(::new("scripts/skills/actives/rf_sling_acid_flask_skill_mb"));  		
  		this.getContainer().getActor().getSkills().getSkillByID("actives.sling_acid_flask").m.FreeCounter++;
	}}.onPutIntoBag;
